...
public class StudentDAOImpl implements StudentDAOInterface {
	...
	public ArrayList<Course> fetchRegisteredCourses(String studentId, int sem) throws SQLException {
		...
		PreparedStatement stmt = instanceDAO.conn.prepareStatement("select * from registeredCourse where stuid = ? and semester = ?");
		...
		while (myRs.next()) {
            ...
            while(newRs.next()) {
            	...
            	courseFound.setCourseID(newRs.getInt("courseid"));
	...
	@Override
	public boolean payFee(String studentId, int modeOfPayment) throws SQLException {
	...
	public boolean isFeePaid(String studentId, int sem) throws SQLException {
		...
		PreparedStatement stmt = instanceDAO.conn.prepareStatement("select * from payment where stuid = ? and semester = ?");
		stmt.setInt(1, Integer.parseInt(studentId));
		stmt.setInt(2, sem);
		ResultSet myRs = stmt.executeQuery();
	...
	public ArrayList<Student> getAllStudents(String session) throws SQLException {
		...
		while (myRs.next()) {
            ...
            currstudent.setStudentID(myRs.getInt(1));
